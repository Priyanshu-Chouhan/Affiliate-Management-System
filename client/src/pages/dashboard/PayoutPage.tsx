...
export const PayoutPage = () => {
  ...
  const loadData = () => {
    affiliateApi.getDashboard().then((res) => setBalance(res.data.data.availableBalance));
  ...
  return (
    <div style={{ maxWidth: 700, margin: '40px auto', padding: 24 }}>
      ...
      <div style={{ padding: 16, background: '#f0f9ff', borderRadius: 8, marginBottom: 24 }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: 32 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input type="number" placeholder="Amount" value={amount} min={0} step="0.01" required
            onChange={(e) => setAmount(e.target.value)} style={{ flex: 1, padding: 8 }} />
          <button type="submit" disabled={loading}>{loading ? 'Submitting...' : 'Request'}</button>
        </div>
        ...
        <tbody>
          {history.map((p) => (
            <tr key={p.id}>
              ...
              <td style={{ padding: 10, borderBottom: '1px solid #eee' }}>{p.processedAt ? new Date(p.processedAt).toLocaleDateString() : '—'}</td>
            </tr>
          ))}
          {history.length === 0 && (
            <tr><td colSpan={4} style={{ padding: 20, textAlign: 'center', color: '#999' }}>No payout requests yet</td></tr>
          )}
        </tbody>
      </table>
    </div>
